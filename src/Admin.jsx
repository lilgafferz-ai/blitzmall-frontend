    ...
    <div className="ad">
      <header className="ad-header">
        ...
        <div className="ad-head-right">
          <button className={"ad-bell" + (alerts.out.length ? " ring" : "")} onClick={() => setMuted(m => !m)}>
            {muted ? "🔕" : "🔔"}{totalAlerts > 0 && <i className="ad-bell-dot">{totalAlerts}</i>}
          </button>
      ...
      {showBanner && totalAlerts > 0 && (
        <div className={"ad-alert-banner" + (alerts.out.length || (alerts.expired||[]).length ? " urgent" : "")}>
        ...
        <div className="ad-body pos-wrap">
          <div className="pos-left">
            ...
            <form className="pos-scan" onSubmit={handleScanSubmit}>
              ...
              <button className="ad-btn small" type="submit">Find</button>
            </form>
            ...
            <div className="pos-cart">
              {saleCart.length === 0 ? <p className="ad-empty">Scan or search to add items.</p> : saleCart.map(i => {
          ...
          <div className="pos-right">
            {receipt ? (
              <div className="receipt-box">
                ...
                <div className="receipt-actions">
                  <button className="ad-btn small" onClick={() => window.print()}>🖨 Print</button>
                  {receiptWALink && <a className="cr-remind" href={receiptWALink} target="_blank" rel="noreferrer">📱 WhatsApp</a>}
                  <button className="ad-btn small" onClick={() => setReceipt(null)}>New sale</button>
                </div>
              </div>
            ) : (
              <>
            ...
            <div className="pos-recent">
              <h3>Recent sales</h3>
              {recentSales.length === 0 ? <p className="ad-empty sm">No sales yet.</p> : recentSales.map(s => (
                <div className="pos-recent-row" key={s._id}>
                  <div><b>{money(s.total)}</b><span className="ad-muted"> · {s.paymentMethod} · {s.staff||""} · {new Date(s.createdAt).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span></div>
      ...
      {tab === "inventory" && (
        <div className="ad-body">
          ...
          {showForm && (
            <form className="ad-form" onSubmit={submitProduct}>
              ...
              <label className="ad-full">Description<textarea value={form.description} onChange={e => setForm(s=>({...s,description:e.target.value}))} placeholder="Optional notes for customers" /></label>
          ...
          {filtered.length === 0 ? <p className="ad-empty">No items yet.</p> : (
            <div className="ad-list">{filtered.map(p => {
              ...
              return (
                <div className="ad-item" key={p._id}>
                  ...
                  <div className="ad-item-main">
                    <div className="ad-item-top"><b>{p.name}</b>{stockTag(p.stock)}{expiryTag(p.expiryDate)}</div>
                    <div className="ad-item-sub"><span className="ad-cat">{p.category||"Other"}</span>{p.barcode && <span className="ad-bc">#{p.barcode}</span>}</div>
                    <div className="ad-item-prices"><span>Buy {money(p.buyingPrice||0)}</span><span>Sell {money(p.price||0)}</span><span className={margin>=0?"profit":"loss"}>Margin {money(margin)}</span></div>
                  </div>
                  <div className="ad-item-actions"><button onClick={() => editProduct(p)}>✏️</button><button onClick={() => delProduct(p._id)}>🗑️</button></div>
                </div>
              );
            })}</div>
          )}
      ...
      {tab === "orders" && (
        <div className="ad-body"><h2>Orders</h2>
      ...
      {tab === "records" && (
        <div className="ad-body">
          <div className="ad-row-between"><h2>Records</h2><div style={{display:"flex",gap:10}}><button className="ad-btn small" onClick={loadSummary}>↻ Refresh</button><button className="ad-btn small" onClick={exportBackup}>⬇ Backup</button></div></div>
            ...
            <>
              ...
              <div className="rec-two">
                ...
                <div className="rec-panel"><h3>⚠ Alerts</h3>
                  {[...(summary.expired||[]),...summary.out,...(summary.expiringSoon||[]),...summary.low].length === 0 ? <p className="ad-empty sm">All good.</p> : (<>
      ...
      {tab === "expenses" && (
        <div className="ad-body"><h2>Expenses</h2>
          ...
          <form className="exp-form" onSubmit={addExpense}><input value={expDesc} onChange={e => setExpDesc(e.target.value)} placeholder="What for? e.g. Transport, Rent" required /><input type="number" step="0.01" value={expAmount} onChange={e => setExpAmount(e.target.value)} placeholder="Amount KES" required /><button className="ad-btn small" type="submit">Add</button></form>
          {expenses.length === 0 ? <p className="ad-empty">No expenses yet.</p> : <div className="ad-list">{expenses.map(x => <div className="exp-row" key={x._id}><div><b>{x.description}</b><span className="ad-muted"> · {new Date(x.createdAt).toLocaleDateString()}</span></div><div className="exp-right"><b className="red">{money(x.amount)}</b><button className="pos-void" onClick={() => delExpense(x._id)}>delete</button></div></div>)}</div>}
        </div>
      )}
      ...
      {tab === "reviews" && (() => {
        ...
        return (
          <div className="ad-body"><h2>Reviews</h2>
            ...
            {count === 0 ? <p className="ad-empty">No reviews yet.</p> : <div className="ad-list">{reviews.map(r => <div className={"rv-row" + (r.rating<=2?" bad":"")} key={r._id}><div className="rv-info"><div className="rv-top"><span className="rv-stars">{stars(r.rating)}</span><b>{r.customerName}</b></div>{r.message && <p className="rv-msg">{r.message}</p>}<span className="cr-date">{new Date(r.createdAt).toLocaleDateString()}</span></div><button className="pos-void" onClick={() => delReview(r._id)}>delete</button></div>)}</div>}
          </div>
        );
      })()}

      {tab === "staff" && (
        <div className="ad-body"><h2>Staff & Cashiers</h2>
          <p className="ad-muted" style={{marginBottom:16}}>Add staff here. They appear in the cashier selector on the Sell tab so each sale is attributed correctly. Full secure staff logins come with the security/deployment phase.</p>
          <form className="exp-form" onSubmit={addStaff}><input value={newStaffName} onChange={e => setNewStaffName(e.target.value)} placeholder="Staff name e.g. Jane" required /><button className="ad-btn small" type="submit">Add cashier</button></form>
          {staffList.length === 0 ? <p className="ad-empty">No staff added yet.</p> : <div className="ad-list">{staffList.map(s => <div className="exp-row" key={s._id}><div><b>{s.name}</b><span className="ad-muted"> · {s.role||"Cashier"} · Added {new Date(s.createdAt).toLocaleDateString()}</span></div><button className="pos-void" onClick={() => delStaff(s._id)}>remove</button></div>)}</div>}
        </div>
      )}
    </div>
  );
